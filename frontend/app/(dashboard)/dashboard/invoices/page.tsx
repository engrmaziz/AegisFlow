                        ...
                        <tbody>
                            {loading ? (
                                ...
                                filteredInvoices.map((inv: any) => {
                                    ...
                                    const daysLate = isOverdue ? Math.floor((new Date().getTime() - new Date(inv.due_date).getTime()) / (1000 * 3600 * 24)) : 0;
                                    ...
                                    return (
                                        <tr key={inv.id} className="border-b border-border hover:bg-muted/30 transition-colors">
                                            ...
                                            <td className="px-6 py-4">
                                                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${displayStatus === 'Paid' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400' :
                                                    displayStatus === 'Overdue' ? 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400' :
                                                        'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400'
                                                    }`}>
                                                    {displayStatus} {isOverdue && daysLate > 0 && <span className="ml-1 opacity-80">({daysLate}d late)</span>}
                                                </span>
                                            </td>
                                            <td className="px-6 py-4">
