...
export default function FinancialAdvisor() {
    ...
    const [messages, setMessages] = useState<Message[]>([
        { role: 'assistant', content: 'Hello! I am your AegisFlow AI Financial Advisor. How can I assist you with your cash flow forecasting or risk profiling today?', timestamp: new Date().toLocaleTimeString() }
    ]);
    ...
    const handleSend = async (e?: React.FormEvent) => {
        ...
        try {
            ...
            setMessages(prev => [...prev, { role: 'assistant', content: data.answer, timestamp: new Date().toLocaleTimeString() }]);
        } catch (error) {
    ...
    const handleDownloadChat = () => {
        ...
        pdf.text("InvoiceIQ - AI Financial Advice", margin, yPos);
        ...
        messages.forEach((msg) => {
            ...
            pdf.text(`${roleText} (${timeText})`, margin, yPos);
            yPos += 6;
            ...
            // Split text to fit width and clean raw markdown out of PDF log format
            const cleanContent = msg.content.replace(/\*\*/g, '').replace(/__/g, '').replace(/###/g, '').replace(/##/g, '').replace(/#/g, '').replace(/-/g, '•');
        ...
        for (let i = 1; i <= pageCount; i++) {
            ...
            pdf.text(`Report generated by InvoiceIQ — https://iq-invoice.vercel.app/ | ${dateStr}`, margin, pageHeight - 10);
        }
                        ...
                        <div className="flex items-center justify-between p-4 border-b border-white/10 bg-gradient-to-r from-blue-600/20 to-purple-600/20 backdrop-blur-md">
                            ...
                            <div className="flex items-center gap-2">
                                <button
                                    ...
                                    className="flex items-center gap-1.5 text-xs font-medium bg-blue-500/10 text-blue-400 hover:bg-blue-500/20 hover:text-blue-300 transition-colors px-2.5 py-1.5 rounded"
                                            ...
                                            {msg.role === 'user' ? (
                                                ...
                                                <ReactMarkdown
                                                    ...
                                                    components={{
                                                        ...
                                                        h1: ({ node, ...props }) => <h1 className="text-white font-bold my-2 text-base" {...props} />,
                                                        h2: ({ node, ...props }) => <h2 className="text-white font-bold my-2 text-sm" {...props} />,
                                                        h3: ({ node, ...props }) => <h3 className="text-white font-bold my-2 text-sm" {...props} />,
                                                        ul: ({ node, ...props }) => <ul className="list-disc leading-relaxed ml-5 text-slate-300 space-y-1 my-2" {...props} />,
                                                        ol: ({ node, ...props }) => <ol className="list-decimal leading-relaxed ml-5 text-slate-300 space-y-1 my-2" {...props} />,
